...
/* ── Cards QClay ── */
.qc-card{
    background:#fff;border-radius:18px;border:1px solid #e2ddd8;
    box-shadow:0 2px 16px rgba(26,23,20,0.06),0 1px 4px rgba(26,23,20,0.04);
    transition:box-shadow .2s,transform .2s;
}
.qc-card:hover{
    box-shadow:0 6px 28px rgba(26,23,20,0.1),0 2px 8px rgba(26,23,20,0.06);
...
def card(content_html: str, padding: str = "18px 20px", extra_style: str = "") -> str:
    """Gera HTML de um card QClay padrão."""
    return (
        f'<div style="background:#fff;border-radius:18px;padding:{padding};'
        f'box-shadow:0 2px 16px rgba(26,23,20,0.07),0 1px 4px rgba(26,23,20,0.04);'
...
with tab1:
    ...
    with h1:
        ...
        for sv, sl, sc in stats_hero:
            stats_html += (
                f'<div style="background:#f7f4f0;border-radius:14px;padding:12px 16px;text-align:center;min-width:80px;">'
        ...
        st.markdown(
            f'<div style="background:#fff;border-radius:18px;padding:20px 24px;'
            f'box-shadow:0 2px 16px rgba(26,23,20,0.07),0 1px 4px rgba(26,23,20,0.04);'
    ...
    with h2:
        # Card: Saldo do Período Selecionado por Ano
        # Agrupa os períodos selecionados por ano e mostra círculos concêntricos
        df_hero = df[df["data"].dt.to_period("M").isin(periodos_sel)].copy()
        df_hero["ano"] = df_hero["data"].dt.year
        anos_hero_disp = sorted(df_hero["ano"].dropna().unique(), reverse=True)

        lucros_periodo = []
        for ano in anos_hero_disp:
            sub = df_hero[df_hero["ano"]==ano]
            r = sub[sub["tipo"]=="Receita"]["valor"].abs().sum()
            d = sub[sub["tipo"]=="Despesa"]["valor"].abs().sum()
            lucros_periodo.append((ano, r-d))
        lucros_periodo.sort(key=lambda x: x[1], reverse=True)

        circles_html = ""
        tamanhos   = [90, 68, 50, 34]
        opacidades = ["#c9a96e22","#c9a96e44","#c9a96e77","#c9a96e"]
        for i, (ano, lucro_a) in enumerate(lucros_periodo[:4]):
            sz      = tamanhos[i]
            bg      = opacidades[i]
            cor_txt = "#fff" if i == 3 else "#8a7a60"
            font_sz = "8px" if i < 3 else "9px"
            circles_html += (
                f'<div style="position:absolute;width:{sz}px;height:{sz}px;border-radius:50%;'
                f'background:{bg};top:50%;left:50%;transform:translate(-50%,-50%);'
                f'display:flex;align-items:center;justify-content:center;'
                f'font-size:{font_sz};font-weight:700;color:{cor_txt};">'
                f'{fmt_k(lucro_a)}</div>'
            )
        ...
        st.markdown(
            f'<div style="background:#fff;border-radius:16px;padding:16px 20px;'
            f'box-shadow:0 2px 12px rgba(26,23,20,0.06);border:1px solid #e0dbd4;height:100%;">'
            ...
            f'<div style="position:relative;height:90px;">{circles_html}</div>'
        ...
        if mini_bars:
            ...
            for i, b in enumerate(mini_bars):
                ...
                bars_html += (
                    ...
                    f'background:{cor_icon};opacity:{alpha};height:{h}%;"></div>'
                )
            bars_html += '</div>'
        sub_html = f'<div style="font-size:10px;color:#a09080;margin-top:4px;font-weight:500;">{sub}</div>' if sub else ""
        with col:
            st.markdown(
                f'<div style="background:#fff;border-radius:18px;padding:18px 18px 14px;'
                f'box-shadow:0 2px 16px rgba(26,23,20,0.07),0 1px 4px rgba(26,23,20,0.04);'
                ...
                f'<div style="width:36px;height:36px;border-radius:10px;background:{bg_icon};'
                f'display:flex;align-items:center;justify-content:center;font-size:15px;color:{cor_icon};font-weight:800;">{icone}</div>'
    ...
    with g1:
        ...
        fig.add_bar(name="Receita", x=ex, y=rv, width=0.22,
                    marker=dict(color=C["sage"], opacity=0.85, line=dict(width=0), cornerradius=4))
        fig.add_bar(name="Despesa", x=ex, y=dv, width=0.22,
                    marker=dict(color=C["terra"], opacity=0.85, line=dict(width=0), cornerradius=4))
        fig.add_scatter(name="Saldo", x=ex, y=lucro_v, mode="lines+markers",
                        line=dict(color=C["amber"], width=3),
                        marker=dict(size=8, color=C["amber"], line=dict(color="#fff",width=2)))
    ...
    with g2:
        ...
        rows_ef = (
            ...
            f'<div style="width:34px;height:34px;border-radius:10px;background:#edf7eb;'
            f'display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:800;color:{margem_cor};flex-shrink:0;">%</div>'
            f'<div style="flex:1;">'
            f'<div style="font-size:11px;font-weight:600;color:#1a1714;">Margem Líquida</div>'
            f'<div style="height:4px;background:#f0ece8;border-radius:2px;margin-top:5px;">'
            f'<div style="height:100%;width:{min(M["margem"],100):.1f}%;background:{margem_cor};border-radius:2px;"></div>'
            f'</div></div>'
            f'<div style="font-size:14px;font-weight:800;color:{margem_cor};flex-shrink:0;">{M["margem"]:.1f}%</div>'
            f'</div>'
            # Comprometimento
            f'<div style="display:flex;align-items:center;gap:10px;padding:9px 0;border-bottom:1px solid #f5f1ec;">'
            f'<div style="width:34px;height:34px;border-radius:10px;background:{comp_bg};'
            f'display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:800;color:{comp_cor};flex-shrink:0;">↓</div>'
            f'<div style="flex:1;">'
            f'<div style="font-size:11px;font-weight:600;color:#1a1714;">Comprometimento</div>'
            f'<div style="height:4px;background:#f0ece8;border-radius:2px;margin-top:5px;position:relative;">'
            f'<div style="height:100%;width:{min(comp,100):.1f}%;background:{comp_cor};border-radius:2px;"></div>'
            f'<div style="position:absolute;top:-3px;left:60%;width:1.5px;height:10px;background:rgba(201,133,110,0.4);"></div>'
            f'</div></div>'
            ...
            f'<div style="width:34px;height:34px;border-radius:10px;background:#f5f2ee;'
            f'display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:800;color:{c};flex-shrink:0;">{'+'  if l>=0 else '-'}</div>'
            f'<div style="flex:1;">'
        ...
        st.markdown(
            ...
            f'box-shadow:0 2px 12px rgba(26,23,20,0.06);border:1px solid #e0dbd4;">'
            f'{rows_ef}'
    ...
    with g3:
        ...
        st.markdown(
            f'<div style="background:#fff;border-radius:16px;padding:14px 16px;'
            f'box-shadow:0 2px 12px rgba(26,23,20,0.06);border:1px solid #e0dbd4;">'
    ...
    with a1:
        ...
        for _, row in df_recentes.iterrows():
            ...
            rows_a += (
                ...
                f'<div style="width:34px;height:34px;border-radius:12px;background:{cor_icon_bg};'
                f'display:flex;align-items:center;justify-content:center;font-size:13px;'
                f'font-weight:800;color:{cor_icon_fg};flex-shrink:0;">{inicial}</div>'
    ...
    with a2:
        ...
        dist_html = ""
        for cat, val in desp_cat.sort_values(ascending=False).head(5).items():
            ...
            dist_html += (
                ...
                f'</div>'
                f'<div style="height:4px;background:#f0ece8;border-radius:2px;">'
                f'<div style="height:100%;width:{pct_desp:.1f}%;background:{cor_bar};border-radius:2px;"></div>'
                f'</div></div>'
            )
        st.markdown(
            f'<div style="background:#fff;border-radius:16px;padding:16px 18px;'
...
with tab2:
    ...
    for col, lbl, val, cor in [
        ...
        with col:
            ...
            st.markdown(
                ...
                f'<div style="width:34px;height:34px;border-radius:10px;background:{bg_icon};'
                f'display:flex;align-items:center;justify-content:center;font-size:16px;font-weight:800;color:{cor};">{icone}</div>'
                f'<div style="font-size:9px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:#a09080;">{lbl}</div>'
                f'</div>'
    ...
    fig2.add_scatter(x=dr, y=rec_d.values, name="Receita", fill="tozeroy",
                     ...
                     marker=dict(size=4,color=C["sage"],line=dict(color="#fff",width=1.5)))
    fig2.add_scatter(x=dr, y=desp_d.values, name="Despesa", fill="tozeroy",
                     ...
                     marker=dict(size=4,color=C["terra"],line=dict(color="#fff",width=1.5)))
    ...
    if len(dr) > 0:
        fig3.add_scatter(x=[dr[-1]], y=[saldo_final], mode="markers+text", showlegend=False,
                         ...
                         marker=dict(size=12,color=cor_s,line=dict(color="#fff",width=2.5)))
...
with tab3:
    ...
    with col_l:
        ...
        rec_cat = df_mes[df_mes["tipo"]=="Receita"].groupby("categoria")["valor_abs"].sum().reset_index()
        fig_r   = go.Figure(go.Pie(
            labels=rec_cat["categoria"], values=rec_cat["valor_abs"], hole=0.55,
            marker=dict(colors=C["seq"], line=dict(color="#fff",width=2)),
            textfont=dict(family="Outfit",size=11,color="#1a1714"),
            pull=[0.02]*len(rec_cat),
        ))
        fig_r.update_layout(**{k:v for k,v in PLOT.items() if k not in ["xaxis","yaxis"]}, height=260)
    ...
    with col_r:
        ...
        fig_d.update_layout(**{k:v for k,v in PLOT.items() if k!="xaxis"}, height=260)
        fig_d.update_xaxes(tickformat=",.0f")
    ...
    fig_pct = go.Figure(go.Bar(
        ...
        marker=dict(color=cores_pct, opacity=0.85, line=dict(width=0), cornerradius=4),
    ...
    top_cats = desp_cat.sort_values(ascending=False).head(3).index.tolist()
    df_evol  = df[df["tipo"]=="Despesa"].copy()
    df_evol["valor_abs"] = df_evol["valor"].abs()
    df_evol["mes"]       = df_evol["data"].dt.to_period("M").astype(str)
    df_evol = df_evol[df_evol["categoria"].isin(top_cats)]
    df_evol = df_evol.groupby(["mes","categoria"])["valor_abs"].sum().reset_index()

    fig_ev = go.Figure()
    ev_c = [C["terra"],C["amber"],C["sage"]]
    for i, cat in enumerate(top_cats):
        sub = df_evol[df_evol["categoria"]==cat]
        fig_ev.add_scatter(x=sub["mes"], y=sub["valor_abs"], name=cat, mode="lines+markers",
                           line=dict(color=ev_c[i],width=2.5),
                           marker=dict(size=8,color=ev_c[i],line=dict(color="#fff",width=2)))
...
with tab5:
    ...
    def meta_card(col, titulo, atual, meta, unidade="R$", inverso=False):
        ...
        with col:
            st.markdown(
                f'<div style="background:#fff;border-radius:18px;padding:18px;'
                f'box-shadow:0 2px 16px rgba(26,23,20,0.07);border:1px solid #e8e3dc;'
                ...
                f'<div style="height:6px;background:#f0ece8;border-radius:3px;margin:12px 0 8px;position:relative;">'
                f'<div style="height:100%;width:{bar_pct:.1f}%;background:{cor_bar};border-radius:3px;transition:width .4s;"></div>'
                f'</div>'
    ...
    for i, cat in enumerate(df_cats_ordenadas):
        ...
        with cols_meta_cat[i % 3]:
            st.markdown(
                ...
                f'box-shadow:0 1px 8px rgba(26,23,20,0.06);border:1px solid #e8e3dc;margin-bottom:10px;">'
                ...
                f'<div style="height:4px;background:#f0ece8;border-radius:2px;margin-top:10px;">'
                f'<div style="height:100%;width:{min(pct_c,100):.1f}%;background:{cor_c};border-radius:2px;"></div>'
                f'</div>'
...
with tab7:
    ...
    for col, lbl, val, cor, bg_ic, icone in an_cfgs:
        with col:
            st.markdown(
                ...
                f'<div style="position:absolute;top:0;left:0;right:0;height:3px;background:{cor};border-radius:18px 18px 0 0;opacity:.5;"></div>'
                f'<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px;">'
                f'<div style="width:34px;height:34px;border-radius:10px;background:{bg_ic};'
                f'display:flex;align-items:center;justify-content:center;font-size:15px;font-weight:800;color:{cor};">{icone}</div>'
                f'<div style="font-size:9px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:#a09080;">{lbl}</div>'
                f'</div>'
    ...
    fig_an.add_scatter(name="Saldo", x=ex, y=sv, mode="lines+markers",
                       line=dict(color=C["amber"],width=3),
                       marker=dict(size=9,color=C["amber"],line=dict(color="#fff",width=2)))
        ...
        for i, ano in enumerate(sorted(anos_sel)):
            ...
            if gran == "Anual":
                ...
                fig_ac.add_scatter(x=[ano], y=[r-d], name=ano, mode="markers+text",
                                   marker=dict(size=16,color=cor,line=dict(color="#fff",width=3)),
                ...
                fig_ac.add_scatter(x=xs, y=ys, name=ano, mode="lines+markers",
                                   ...
                                   marker=dict(size=7,color=cor,line=dict(color="#fff",width=2)),
                                   fill="tozeroy",
        ...
        for i, ano in enumerate(sorted(anos_sel)):
            ...
            if gran == "Anual":
                ...
                fig_mg.add_bar(x=[ano], y=[mg], name=ano,
                               marker=dict(color=cor,opacity=0.85,line=dict(width=0),cornerradius=4))
            else:
                gm = df_ano.groupby(["mnum","mes","tipo"])["valor_abs"].sum().reset_index()
                xs,ys = [],[]
                for mn in sorted(gm["mnum"].unique()):
